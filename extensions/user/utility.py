...
from extensions.user.moderation import request_discord_user
...
class snipedMessage():
    ...
    def __init__(self,message):
        ...
        # set the Current time as the deleted_at self variable (We do this beacuase we will only create an instance of this class once a message is deleted. Which means we can track when the message was deleted.)
    ...
    def __repr__(self):
        ...
        return f'<snipedMessage Instance, author={str(self.author)} channel={str(self.channel)} deleted_at={self.deleted_at}>'
...
class utility(commands.Cog):
    ...
    async def _snipe(self,ctx):
        ...
        try:
            ...
            # create the embed 
            emb = discord.Embed(title=f'🔫 SNIPED MESSAGE', description=f'{sniped.content}', color=0x2F3136)
    ...
    async def _botinfo(self,ctx):
        ...
        # get the user count of the bot
        # we turn the get all members method to a set so it doesnt repeat any members that could be in more than 1 server.
        userCount = len(set(self.client.get_all_members()))

        # create the embed to send and add all the fields for the previos variables
        emb = discord.Embed(title=f'{self.client.user.name} Stats', timestamp=datetime.datetime.utcnow())
        emb.add_field(name='Client Version', value=self.client.version, inline=True)
        emb.add_field(name='Python Version', value=self.client.pyversion, inline=True)
        emb.add_field(name='Discord.py Version', value=self.client.dpyversion, inline=True)
        emb.add_field(name='Total Guilds', value=serverCount, inline=True)
        emb.add_field(name='Total Users', value=userCount, inline=True)

        # add the current prefix to the embed
        emb.add_field(name="Client Uptime", value=f"{h.naturaldelta(self.client.start_time)}")

        # add the client owner to the embed
        emb.add_field(name='Bot Creator', value=f"<@393165866285662208>")
        ...
        emb.add_field(name='Github Page', value='[github.com/...](https://github.com/BlutopiaHub/Blutonium-2.0)', inline=True)
        
        # add the bot invite link to the embed
        emb.add_field(name='Invite Link', value=f'[discord.com/...](https://discord.com/oauth2/authorize?client_id={self.client.user.id}&scope=bot&permissions=8)')
    ...
    async def _uptime(self,ctx):
        ...
        # create the embed
        emb = discord.Embed(title='Uptime', description =f"The client has been running for **{h.precisedelta(self.client.start_time)}**", timestamp=datetime.datetime.utcnow() )
        ...
        for ext in self.client.extensions:
            ...
            try:
                ...
                emb.add_field(name=ext.split('.')[2].capitalize(), value=f'Loaded **{h.naturaltime(exts[ext]["start_time"])}**\n**{ext.split(".")[1].capitalize()}** extension\n\_\_\_\_\_\_\_\_',inline=True)
    ...
    async def _ping(self,ctx):
        ...
        # create the embed
        emb = discord.Embed(title='⏳Ping', description=f'🌐 Websocket Latency: `{ping} ms`\n💬 Message Latency: `{pong} ms`\n🐘 Database Latency: `{peng} ms`', color=0x2F3136)
    ...
    async def _invite(self,ctx):
        ...
        emb = discord.Embed(title=f'Invite {self.client.user}', timestamp=datetime.datetime.utcnow(), colour=0x2F3136, description=f'[Invite me to your server!](https://discordapp.com/oauth2/authorize?client_id={self.client.user.id}&scope=bot&permissions=8)\n[Join the Blutonium updates and support server.](https://discord.gg/NNfD6eQ)')
