...
from blutopia.utils import guildembed
...
class owner(commands.Cog, name='Owner'):
    ...
    async def _blacklist(self, ctx):
        ...
        if ctx.invoked_subcommand is None:
            ...
            for uid in blacklist:
                ...
                user = get(self.client.get_all_members(), id=uid)
    ...
    async def _blacklistRem(self, ctx, userid: int):

        # try and fetch the user through the ID
        user = get(self.client.get_all_members(), id=userid)
    ...
    async def _blacklistAdd(self, ctx, userid: int):
    ...
    # dev -> sql subcommand
    @_developer.command(name='sql', aliases=['pg', 'pgsql', 'postgres'],
                        help='Run SQL queries and statments in discord!')
    ...
    # dev -> edit subcommand
    @_developer.command(name='edit', aliases=['ed'],
                        help="Edit the last sent message from the bot using the dev/say command")
    ...
    async def _prefix(self, ctx, *args):
        ...
        self.client.update_prefix(ctx.guild.id, new)
    ...
    async def _guilds(self, ctx, *args):
        ...
        try:
            userid = int(args[0])
            guild = get(allguilds, id=userid)
        except ValueError:
            userid = " ".join(args)
            guild = get(allguilds, name=userid)
