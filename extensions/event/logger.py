...
class logger(commands.Cog, name="logger"):
    ...
    async def on_voice_state_update(self,usr:discord.Member,before,after):
        ...
        if after.channel == before.channel:
            ...
            emb1 = discord.Embed(title = f'{usr}',timestamp=datetime.datetime.utcnow(),colour=0x2F3136)
            emb2 = discord.Embed(title = f'{usr}',timestamp=datetime.datetime.utcnow(),colour=0x2F3136)
            emb3 = discord.Embed(title = f'{usr}',timestamp=datetime.datetime.utcnow(),colour=0x2F3136)

            # add the main field to all the embeds
            emb1.add_field(name = "User changed voice channels", value = f"*{before.channel}* -> *{after.channel}*")
            emb2.add_field(name = "User left a voice channel", value = f"{before.channel}")
            emb3.add_field(name = "User joined a voice channel", value = f"{after.channel}")
            ...
            # if we get an error its because the beore channel is None which means our channel variable would be invalid. This also tells us that a user just joined a channel.
            except:
        ...
        if after.mute == before.mute:
            ...
            if after.mute:
                ...
                # make the embed
                emb = discord.Embed(title = f'{usr}',description = 'User server muted',timestamp=datetime.datetime.utcnow(),colour=0x2F3136)
            ...
            # if the updated state is not muted
                ...
                # make the embed
                emb = discord.Embed(title = f'{usr}',description = 'User un-server muted',timestamp=datetime.datetime.utcnow(),colour=0x2F3136)
        ...
        if after.deaf == before.deaf:
            ...
            if after.deaf:
                ...
                # make the embed
                emb = discord.Embed(title = f'{usr}',description = 'User server deafend',timestamp=datetime.datetime.utcnow(),colour=0x2F3136)
            ...
            # if the updated state is not deaf
                ...
                # make the embed
                emb = discord.Embed(title = f'{usr}',description = 'User un-server deafened',timestamp=datetime.datetime.utcnow(),colour=0x2F3136)
    ...
    async def on_message_delete(self, msg):
        ...
        if moderator is None:
            emb = discord.Embed(title=f'{msg.guild}',description=f'{msg.author.mention} deleted their message in {msg.channel.mention}',colour=0x2F3136,timestamp=datetime.datetime.utcnow())
        else:
    ...
    async def on_message_edit(self, before, after):
        ...
        # set the embed
        emb = discord.Embed(title=f'{before.author}',description=f'A message was edited in {before.channel.mention}',colour=0x2F3136,timestamp=datetime.datetime.utcnow())

        # set all the embed components
        emb.add_field(name='Before',value=f'{before.content}',inline=True)
        emb.add_field(name='After',value=f'{after.content}',inline=True)
